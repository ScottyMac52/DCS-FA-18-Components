# Installing DCS-FA-18C-hornet-Components

## Requirements

- DCS World with the F/A-18C Hornet module installed.
...
```text
Saved Games\DCS\Config\Input\FA-18C_hornet
...
1. Download `DCS-FA-18C-hornet-Components-<version>-OVGME.zip` from the repository release.
2. Add the archive to the OvGME configuration whose root is your DCS Saved Games directory.
3. Enable the package.
4. Start DCS and open **Options → Controls → F/A-18C Hornet**.
5. Confirm that the expected device columns contain the assignments listed in [Control mappings](CONTROL-MAPPINGS.md).
6. Open the in-game kneeboard or OpenKneeboard and confirm that the F/A-18C Hornet pages appear in numeric order.

The archive writes only these package areas:

```text
Config\Input\FA-18C_hornet\joystick
Config\Input\FA-18C_hornet\modifiers.lua   (when configured)
...
The build composes DCS-Common's canonical UI Layer with only the devices used by this module. In **Options → Controls**, select **UI Layer** and confirm that the applicable packaged devices have their shared controls. Module-specific bindings remain under **F/A-18C Hornet**.
