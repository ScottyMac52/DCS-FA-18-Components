...
try {
  ...
  if (-not ($entries | Where-Object { $_.StartsWith("${payloadPrefix}Config/Input/FA-18C_hornet/joystick/", [System.StringComparison]::Ordinal) })) {
  ...
  if (-not ($entries | Where-Object { $_.StartsWith("${payloadPrefix}KNEEBOARD/FA-18C_hornet/", [System.StringComparison]::Ordinal) })) {
